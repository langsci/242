...
The representation of cases as a sequence of functional heads (\isi{fseq}) follows from the observation that non-accidental case 
\makebox[\linewidth][s]{\isi{syncretism} targets only adjacent cells of declension \isi{paradigm}s if they are ar-}
...
\draw[dashed,->,>=stealth] (t) [in=-145,out=-115,looseness=2.85]  to (tgt);
\end{forest}
}
...
\draw[dashed,->,>=stealth] (t) [in=-150,out=-120,looseness=2.15]  to (tgt);
\end{forest}
...
Let us synopsize the spell-out formula in \cite{Starke2018}, which is an unambiguous specification of how to lexicalize a grammatical \isi{feature}, i.e. an algorithm for spell-out:  
...
\draw[dashed,->,>=stealth] (t) [in=-150,out=-120,looseness=1.75]  to (tgt);
\end{forest} 
...
[F$_{3}$P, s sep=10pt [...,name=F3] [F$_{3}$P [F$_{3}$][..., name=F2]]]]]]]
 ...
 \draw[dashed,->,>=stealth] (F3) to[out=south west,in=south west,looseness=1.55] (F4);
  \draw[dashed,->,>=stealth] (F4) to[out=south west,in=south west,looseness=1.55] (F5);
   \draw[dashed,->,>=stealth] (F5) [in=-145,out=-125,looseness=1.75]  to (F6);
      \draw[dashed,->,>=stealth] (F2) to[out=south west,in=south west,looseness=1.75] (F3);
 \end{forest}}
...
What is indicated in the glosses in (\pref{ch3:Cz}--\pref{ch3:P}) and later in the text are underlying, ``untruncated'' exponents of theme vowels. \is{thematic suffix}
